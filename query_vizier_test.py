...
class TestQuery(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.vizier.radius = 1.5
        self.source_list = [(187.27832916, 2.05199), 'not_a_source', ('18h 36m 56.3364s', '+38:47:1.291'), (1, 2, 3), 'Vega']
    ...
    def test_create_url_negative_dec(self):
        ...
        url = self.vizier._create_url((1.0, -38.783))
        self.assertTrue(url == "http://vizier.u-strasbg.fr/viz-bin/sed?-c=1.000000-38.783000&-c.rs=1.50")

    def test_create_url_different_radius(self):
        """ Check that different radii still work """
        self.vizier.radius = 10
        url = self.vizier._create_url((1.0, -38.783))
    ...
    def test_source_id_not_provided(self):
        ...
        phot = self.vizier._get_all_dataframes(source_list, source_id=range(len(source_list)))
        self.assertTrue(all(phot['source_id'].unique() == [0, 2]))

    def test_source_id_provided(self):
        source_list = [(187.27832916, 2.05199), 'not_a_source', 'Vega']
        source_id = [-6, '23', 4.5]
        phot = self.vizier._get_all_dataframes(source_list, source_id=source_id)
    ...
    def test_query_vizier_source_id(self):
        data, source_info = self.vizier.query_vizier(self.source_list)
        self.assertTrue(all(source_info['source_id'] == [0, 1, 2, 3, 4]))

    def test_query_vizier_sources(self):
        data, source_info = self.vizier.query_vizier(self.source_list)
        for ii, src_id in enumerate(self.source_list):
    ...
    def test_negative_radius(self):
        with self.assertRaises(IOError):
            self.vizier.query_vizier(self.source_list, radius=-5)
